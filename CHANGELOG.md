...
- **澄清记住选择**：用户对澄清问题的选择被记录（按应用+访客隔离，保留最近20条）；同类问题再次出现时，上次所选选项标注 ⭐（上次的选择）；高度相似的问题连续两次选同一项后，自动按历史选择继续并向用户明说，不再打断。相似度用字符二元组 Dice 系数（≥0.45 标注 / ≥0.6 且同选两次自动），纯确定性无额外 LLM 开销
